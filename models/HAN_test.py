...
class HAN(nn.Module):
    def __init__(self, args, embedding_matrix=None, aspect_embedding_matrix=None):
        ...
        self.aspect_rnn = DynamicRNN(args.embed_dim, args.hidden_dim, num_layers=1, batch_first=True,
                                   dropout=args.dropout, bidirectional=False, rnn_type="LSTM")
        self.sentence_rnn = DynamicRNN(args.hidden_dim * 2 + self.position_dim, args.hidden_dim, num_layers=1,
                                       batch_first=True,
                                       dropout=args.dropout, bidirectional=True, rnn_type="LSTM")
        self.word_W = nn.Parameter(
            torch.Tensor(args.hidden_dim * 2 + self.position_dim + self.args.embed_dim,
                         args.hidden_dim * 2 + self.position_dim + self.args.embed_dim))
        self.word_bias = nn.Parameter(torch.Tensor(args.hidden_dim * 2 + self.position_dim + self.args.embed_dim, 1))
        self.word_weight_proj = nn.Parameter(
            torch.Tensor(args.hidden_dim * 2 + self.position_dim + self.args.embed_dim, 1))
        self.word_attention = BasicAttention(hidden_dim=args.hidden_dim * 2, score_function="basic")
        self.sentence_W = nn.Parameter(
            torch.Tensor(args.hidden_dim * 2 + self.position_dim + self.args.embed_dim,
                         args.hidden_dim * 2 + self.position_dim + self.args.embed_dim))
        self.sentence_bias = nn.Parameter(
            torch.Tensor(args.hidden_dim * 2 + self.position_dim + self.args.embed_dim, 1))
        self.sentence_weight_proj = nn.Parameter(
            torch.Tensor(args.hidden_dim * 2 + self.position_dim + self.args.embed_dim, 1))
    ...
    def forward(self, inputs):
        ...
        aspect_output, (aspect_h_n, _) = self.aspect_rnn(aspect, aspect_len)
        aspect_emb = aspect_h_n[0]
        ...
        word_aspect_emb = torch.zeros((sen_output.size(0), torch.max(x_len), aspect_emb.size(1))).to(
            self.args.device)
        word_aspect_emb[x_len > 0] = aspect_emb.repeat(1, torch.max(x_len) * max_sentence_num).view(
            aspect_emb.size(0) * max_sentence_num, -1, aspect_emb.size(1))[x_len>0]
        # print(aspect_emb.size(), word_position_emd_tmp.size(), word_aspect_emb.size())
        word_position_with_aspect = torch.cat((word_position_emd_tmp, word_aspect_emb), dim=-1)
        # print("sen_output", sen_output)
        # atten_sen_output, _ = self.word_attention(k=sen_output)
        sen_squish = batch_matmul_bias(torch.cat((sen_output, word_position_with_aspect), dim=-1).transpose(0, 1),
        ...
        segment_aspect_emb = aspect_emb.repeat(1, segment_position_emb_tmp.size(1)).view(aspect_emb.size(0), -1,
                                                                                         aspect_emb.size(1))
        segment_position_with_aspect = torch.cat((segment_position_emb_tmp, segment_aspect_emb), dim=-1)
        doc_squish = batch_matmul_bias(torch.cat((doc_output, segment_position_with_aspect), dim=-1).transpose(0, 1),
