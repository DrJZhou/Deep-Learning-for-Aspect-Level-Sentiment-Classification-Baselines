...
base_path_from = sys.path[0] + "/../data/data_orign/SemEval2015-Task12/"
base_path_to = sys.path[0] + "/../data/data_processed/SemEval2015/"
    ...
    def write_out_json(self, filename, instances):
        ...
        for i in instances:
            ...
            for a in i.aspect:
                ...
                # if a.term == "NULL" and int(a.offsets['to'])!=0:
                #     print(a.term, i.text,i.text[int(a.offsets['from']): int(a.offsets['to'])], a.offsets)
...
def standardization():
    ...
    trainfile = base_path_from + "train/ABSA15_RestaurantsTrain/ABSA-15_Restaurants_Train_Final.xml"
    testfile = base_path_from + "test/ABSA15_Restaurants_Test.xml"
...
def clear_data(data):
    ...
    for instance in data:
        ...
        for a in aspect_terms:
            ...
            to_index = int(a['to'])
            aspect_clean = process_text(aspect)
            start_clean = process_text(text[:from_index])
            if to_index == 0:
                opinion_clean.append({'aspect': aspect_clean, 'category': category, 'polarity': polarity, 'from': 0,
                                      'to': 0})
            else:
