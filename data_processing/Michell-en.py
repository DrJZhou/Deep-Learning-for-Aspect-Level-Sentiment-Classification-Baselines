...
base_path_from = sys.path[0] + "/../data/data_orign/Michell/"
base_path_to = sys.path[0] + "/../data/data_processed/Michell/"
...
def validate(filename):
    ...
    id_set = {}
    for i in range(1, 11):
        fr = open(filename + str(i), "r")
        text, aspect, category, polarity, id = "", "", "", "", ""
        for line in fr.readlines():
            data = line.strip()
            if len(data) == 0:
                if len(text) > 0:
                    ss.append(text.strip())
                if len(aspect) > 0:
                    # print(text, aspect, category, polarity)
                    aspects.append(Aspect('', '', '', []).create(text_end=len(text.strip()), term=aspect.strip(),
                                                                 category=category, polarity=polarity).term)
                text, aspect, category, polarity, id = "", "", "", "", ""
                continue
            if data.startswith("## Tweet"):
                id = data.replace("## Tweet", "").strip()
                text, aspect, category, polarity = "", "", "", ""
            else:
                data = line.strip().split("\t")

                if data[2] in ["positive", "neutral", "negative"] and data[1].startswith("B-"):
                    aspect = data[0]
                    category = data[1].replace("B-", "")
                    polarity = data[2]
                elif data[1].startswith("I-") and len(aspect) > 0:
                    aspect += " " + data[0]
                elif len(aspect) > 0:
                    # print(text, aspect, category, polarity)
                    aspects.append(Aspect('', '', '', []).create(text_end=len(text.strip()), term=aspect.strip(),
...
class Aspect:
    ...
    def create(self, text_end, term, category, polarity):
        self.term = term
        self.category = category
        self.polarity = polarity
        self.offsets = {'from': text_end - len(term), 'to': text_end}
        return self
...
class Instance:
    ...
    def __init__(self, element):
        ...
        self.id = element['id']
        self.aspect = [Aspect('', '', '', []).create(text_end=e['text_end'], term=e['term'], category=e["category"],
                                                     polarity=e['polarity']) for e in element["aspects"]]
        ...
        for i in range(1, 11):
            ...
            for line in fr.readlines():
                ...
                if len(data) == 0:
                    if len(aspect) > 0:
                        # print(text, aspect, category, polarity)
                        aspects.append({"text_end": len(text.strip()), "term": aspect.strip(), "category": category,
                                        "polarity": polarity})
                        aspect = ""
                    if len(text) > 0:
                        if not id_set.has_key(id):
                            id_set[id] = 1
                            instances.append({'id': id, 'text': text.strip(), 'aspects': aspects})
                    text, aspect, category, polarity, id = "", "", "", "", ""
                    aspects = []
                    continue
                if data.startswith("## Tweet"):
                    ...
                    if data[2] in ["positive", "neutral", "negative"] and data[1].startswith("B-"):
                        ...
                        # print(text, aspect, category, polarity)
                        aspects.append(
                            {"text_end": len(text.strip()), "term": aspect.strip(), "category": category,
                             "polarity": polarity})
                        aspect = ""
                    text += data[0] + " "
            if len(aspect) > 0:
                # print(text, aspect, category, polarity)
                aspects.append(
                    {"text_end": len(text.strip()), "term": aspect.strip(), "category": category, "polarity": polarity})
                aspect = ""
            if len(text) > 0:
                if not id_set.has_key(id):
                    id_set[id] = 1
                    instances.append({'id': id, 'text': text.strip(), 'aspects': aspects})
        self.corpus = Corpus(instances)
...
def clear_data(data):
    ...
    for instance in data:
        ...
        for a in aspect_terms:
            ...
            opinion_clean.append({'aspect': aspect_clean, 'category': category, 'polarity': polarity,
                                  'from': len(start_clean), 'to': len(start_clean) + len(aspect_clean)})
...
def standardization():
    ...
    trainfile = base_path_from + "en/10-fold/train."
    testfile = base_path_from + "en/10-fold/test."
